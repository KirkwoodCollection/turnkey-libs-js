...
export function isInformational(status: number): boolean {
  return status >= HTTP_STATUS_CATEGORIES.INFORMATIONAL[0] && status <= HTTP_STATUS_CATEGORIES.INFORMATIONAL[1];
}

export function isSuccess(status: number): boolean {
  return status >= HTTP_STATUS_CATEGORIES.SUCCESS[0] && status <= HTTP_STATUS_CATEGORIES.SUCCESS[1];
}

export function isRedirection(status: number): boolean {
  return status >= HTTP_STATUS_CATEGORIES.REDIRECTION[0] && status <= HTTP_STATUS_CATEGORIES.REDIRECTION[1];
}

export function isClientError(status: number): boolean {
  return status >= HTTP_STATUS_CATEGORIES.CLIENT_ERROR[0] && status <= HTTP_STATUS_CATEGORIES.CLIENT_ERROR[1];
}

export function isServerError(status: number): boolean {
  return status >= HTTP_STATUS_CATEGORIES.SERVER_ERROR[0] && status <= HTTP_STATUS_CATEGORIES.SERVER_ERROR[1];
}
...
export function getStatusMessage(status: number): string {
  return HTTP_STATUS_MESSAGES[status as keyof typeof HTTP_STATUS_MESSAGES] || 'Unknown Status';
}